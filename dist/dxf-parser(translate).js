 ...
 * 读取表示DXF文件行的数组。获取一个数组并提供一个简单的接口来提取组代码和值对。
 ...
 * 从数组中获取下一个组（代码、值）。一个组是数组中两个连续的元素。第一个是代码，第二个是值。
 * @returns {{code: Number}|*}
 */
...
DxfParser.prototype._parse = function(dxfString) {
	...
	var parseAll = function() {
		...
		while(!scanner.isEOF()) {
			if(curr.code === 0 && curr.value === 'SECTION') {
				...
				if (curr.value === 'HEADER') { // header 段 包含图形基本信息，由autocad数据库版本号和一些系统变量组
					log.debug('> HEADER');
					dxf.header = parseHeader();
					log.debug('<');
				} else if (curr.value === 'BLOCKS') {  // 块段
                    // 包含构成图形中每个块参照的块定义和图形图元
					log.debug('> BLOCKS');
					dxf.blocks = parseBlocks();
					log.debug('<');
				} else if(curr.value === 'ENTITIES') { // 实体段
                    // 包含图形中的图形对象包括：块参照（插入图元）、实体段记录了每个实体名称、所在图层及其名字、线型、颜色等
					log.debug('> ENTITIES');
					dxf.entities = parseEntities(false);
					log.debug('<');
				} else if(curr.value === 'TABLES') {
				    // 线形表(LTYPE) 层表、字体表、视图表、用户坐标系统表、视窗配置表、标注字体表、申请符号表
					log.debug('> TABLES');
					dxf.tables = parseTables();
					log.debug('<');
				} else if(curr.value === 'EOF') {  // 文件结束
					log.debug('EOF');
				} else {
     ...
     * 在分析器第一次读取0:实体组之后调用。扫描仪应该已经在第一个实体的开头。
