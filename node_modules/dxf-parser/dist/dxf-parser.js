...
function parseGroupValue(code, value) {
	if(code <= 9) return value; // 字符型
	if(code >= 10 && code <= 59) return parseFloat(value); // 双精度三维点
	if(code >= 60 && code <= 99) return parseInt(value); // 16-32位整数
	if(code >= 100 && code <= 109) return value; // 字符串
	if(code >= 110 && code <= 149) return parseFloat(value); // 浮点类型
	...
	if(code >= 300 && code <= 369) return value; // 字符串（包含16进制字符串）
	...
	if(code === 999) return value; // 注释
	if(code >= 1000 && code <= 1009) return value; // 字符串
	if(code >= 1010 && code <= 1059) return parseFloat(value);
	if(code >= 1060 && code <= 1071) return parseInt(value); // 16位-32位整数值
